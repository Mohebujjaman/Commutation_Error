...
N = 30;
DNSval = zeros(N,num_time_steps);
DNSsolution = zeros(size(Snapshots,1),num_time_steps);
...
for ts=1:num_time_steps
    GlobalV = Snapshots(:,snapIndex+ts);
    ...
    for i=1:N
      DNSsolution(:,ts) = DNSsolution(:,ts)+DNSval(i,ts)*PhiR(:,i);
    end
end

save('DNSsolutionRe500d30','DNSsolution');
